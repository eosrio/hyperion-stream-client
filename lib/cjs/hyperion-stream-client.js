...
class HyperionStreamClient {
            ...
            for (let optsKey in options) {
                if (options[optsKey] !== undefined) {
                    ...
                    switch (optsKey) {
                        ...
                        case 'chainApi': {
                            this.options.chainApi = (0, functions_1.trimTrailingSlash)(value);
                            break;
                        }
                        default: {
    ...
    setEndpoint(endpoint) {
        if (endpoint) {
            this.socketURL = (0, functions_1.trimTrailingSlash)(endpoint);
        }
        else {
    ...
    setupIncomingQueue() {
        ...
        this.dataQueue = (0, async_1.queue)((task, taskCallback) => {
            ...
            if (trackedRequest) {
                ...
                if (task.mode === "live" && !trackedRequest.live) {
                    ...
                    this.emit(interfaces_1.StreamClientEvents.DATA, task);
        ...
        this.dataQueue.drain(() => {
            this.emit(interfaces_1.StreamClientEvents.DRAIN);
        });
        this.dataQueue.empty(() => {
            this.emit(interfaces_1.StreamClientEvents.EMPTY);
        });
    }
    setupIrreversibleQueue() {
        ...
        if (this.options.libStream) {
            this.libDataQueue = (0, async_1.queue)((task, callback) => {
                ...
                this.emit(interfaces_1.StreamClientEvents.LIBDATA, task);
    ...
    handleLibUpdate(msg) {
        ...
        this.emit(interfaces_1.StreamClientEvents.LIBUPDATE, msg);
            ...
            return new Promise((resolve, reject) => {
                if (!this.socketURL) {
                    ...
                    this.socket.on('connect', () => {
                        ...
                        this.online = true;
                        this.emit(interfaces_1.StreamClientEvents.CONNECT);
                    ...
                    this.socket.on('fork_event', (msg) => {
                        this.emit(interfaces_1.StreamClientEvents.FORK, msg);
                    });
